<script lang="ts">
  ...
  function onDifficultyChange(index, selectedDifficulty, difficultiesCount, selectedDifficultiesOption) {
  ...
  if (!isDifficultiesCountMoreThanOne || difficulties.length === 0) {
    ...
    for (const difficulty of difficulties) {
      const { selectedDifficulty, difficultiesCount, selectedDifficultiesOption } = difficulty;
      ...
      if (difficultiesCount === 2) {
        multiplier =
          selectedDifficultiesOption === "NormalHard"
            ? (selectedDifficulty === 1 ? 1 : 1.4)
            : (selectedDifficulty === 1 ? 0.7 : 1);
      } else {
        multiplier =
          0.7 + ((selectedDifficulty - 1) / (difficultiesCount - 1)) * (1.4 - 0.7);
      }
...
<div class="container">
  ...
  <form>
    <fieldset>
      <legend>
        Время прохождения с <a href="https://www.howlongtobeat.com/" target="_blank">HowLongToBeat</a>
      </legend>
      <input
        id="walkthroughHours"
        type="number"
    ...
    <fieldset>
      ...
      <input id="isDifficultiesCountMoreThanOne" type="checkbox" bind:checked={isDifficultiesCountMoreThanOne} on:change={handleDifficultiesChange} />
      <label for="isDifficultiesCountMoreThanOne">Да</label>
      {#if isDifficultiesCountMoreThanOne}
        <input type="number" bind:value={difficultiesCount} on:input={handleDifficultiesChange} min="1" max="5"/>
    ...
    <fieldset>
      <legend>
        Сложность по <a href="https://gamefaqs.gamespot.com/" target="_blank">GameFAQs</a>
      </legend>
    ...
    <fieldset>
      <legend>
        Оценки с <a href="https://www.metacritic.com/game/" target="_blank">Metacritic</a>
      </legend>
    ...
    <fieldset>
      ...
      <span id="multiplyers">({walkthroughTimeMultiplier} * {difficultyMultiplier} * {gamefaqsDifficultyMultiplier} * {metascoreMultiplier}) * 10</span>
    </fieldset>
  </form>
  ...
  <details>
    ...
    <KaTeXRenderer formula={
      `Points = \\left( WalkthroughTimeMultiplier \\times DifficultyMultiplier \\times GameFAQsDifficultyMultiplier \\times MetascoreMultiplier \\right) \\times 10`
    } displayMode="display"/>
    Итоговый результат округляется до целого числа:
    <KaTeXRenderer formula={
      `\\left\\lfloor 68.5562 \\right\\rceil = 69 \\\\ \\left\\lfloor 68.317 \\right\\rceil = 68`
    } displayMode="display"/>

    <h3>1. Множитель времени прохождения:</h3>
    Время прохождения игры (<KaTeXRenderer formula={`WalkthroughTime`} displayMode="inline" />) берётся с <a href="https://howlongtobeat.com/" target="_blank">HowLongToBeat.com</a>:
    <KaTeXRenderer formula={`WalkthroughTime = Hours + \\frac{Mins}{60}`} displayMode="display" />
    <KaTeXRenderer formula={`WalkthroughTimeMultiplier = 1 + WalkthroughTime`} displayMode="display" />
    ...
    Если в игре множественный выбор сложности (например, как в <a href="https://youtu.be/VUnvAuauSW4?t=3" target="_blank">Silent Hill 2</a> - Action Level и Riddle Level), то множитель высчитывается как среднее арифметическое значение:
    <KaTeXRenderer formula={
      `\\overline{DifficultyMultiplier} = 
    ...
    Показатель сложности (<KaTeXRenderer formula={`GameFAQsDifficulty`} displayMode="inline" />) 
    и количество проголосовавших (<KaTeXRenderer formula={`GameFAQsVotesCount`} displayMode="inline" />) 
    берутся с <a href="https://gamefaqs.gamespot.com/" target="_blank">GameFAQs</a>. 
    На основе количества проголосовавших рассчитывается небольшая прибавка: 
    <KaTeXRenderer formula={
      `VoteBonus = \\begin{cases}
    ...
    <KaTeXRenderer formula={
      `GameFAQsDifficultyMultiplier = 1 + \\left(-0.2 + \\frac{GameFAQsDifficulty - 1}{4} \\times \\left(-0.2 + 0.75\\right)\\right) + voteBonus`
    } displayMode="display" />

    <h3>4. Множитель рейтинга:</h3>
    Множитель рейтинга рассчитывается на основе среднего балла критиков и юзеров с <a href="https://metacritic.com/games" target="_blank">Metacritic</a>:
    <KaTeXRenderer formula={
      `MetascoreMultiplier = 1 + \\frac{85 - \\left( \\frac{CriticsScore + UsersScore}{2} \\right)}{150}`
    } displayMode="display" />
